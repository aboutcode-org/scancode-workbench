...
const LicenseDetections = () => {
  ...
  const [activeLicenseDetection, setActiveLicenseDetection] =
    useState<LicenseDetectionDetails | null>(null);
  ...
  useEffect(() => {
    ...
    if (foundDetection) setActiveLicenseDetection(foundDetection);
  }, [searchParams]);
  ...
  useEffect(() => {
    db.sync.then(async () => {
      ...
      if (queriedDetectionIdentifier && queriedDetection) {
        ...
        setActiveLicenseDetection(queriedDetection);
      } else {
        setActiveLicenseDetection(newLicenseDetections[0]);
      }
    });
  }, []);
  ...
  if (!licenseDetections.length)
    return <NoDataFallback text="No license detections :/" />
  ...
  return (
    <div className="license-detections-main-container">
      ...
      <Allotment className="license-container">
        ...
        >
          {<LicenseDetectionEntity licenseDetection={activeLicenseDetection} />}
        </Allotment.Pane>
      </Allotment>
    </div>
  );
