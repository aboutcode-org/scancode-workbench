...
const columnCompareFn = (col1: ColDef, col2: ColDef) =>
  col1.sortIndex - col2.sortIndex;
...
const CustomColumnSelector = (props: ColumnSelectorProps) => {
  ...
  function selectAllChanged(checked: boolean) {
    if (checked) setColumnDefs([...COLUMN_GROUPS.ALL]);
    else setColumnDefs([]);
  }
    ...
    if (checked) {
      if (!columnDefs.includes(column)) {
        setColumnDefs((prevCols) =>
          [...prevCols, column].sort(columnCompareFn)
        );
      }
    } else {
  ...
  return (
    <Modal size="lg" centered backdrop={true} show={show} onHide={hide}>
      <Modal.Body className="column-selector">
        ...
        <Row>
          <Col>
            {COLUMN_GROUPS.ALL.slice(0, COLUMN_GROUPS.ALL.length / 2).map(
              (column) => (
                <Form.Check
                  type="checkbox"
                  key={column.field}
          ...
          <Col>
            {COLUMN_GROUPS.ALL.slice(COLUMN_GROUPS.ALL.length / 2).map(
              (column) => (
                <Form.Check
                  type="checkbox"
                  key={column.field}
